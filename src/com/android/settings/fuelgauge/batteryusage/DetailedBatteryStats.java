...
public class DetailedBatteryStats extends DashboardFragment {
    ...
    private void updateSummaryStats(@NonNull BatterySummaryStats stats) {
        ...
        setSummary(KEY_SCREEN_ON_DRAIN_RATE,
                formatDrainRate(stats.screenOnDischargePercent, stats.screenOnTimeMs));
        ...
        setSummary(KEY_SCREEN_OFF_DRAIN_RATE,
                formatDrainRate(stats.screenOffDischargePercent, stats.screenOffTimeMs));
    ...
    private String formatDrainRate(int drainPercent, long durationMs) {
        if (drainPercent <= 0 || durationMs <= 0) {
            return getString(R.string.battery_stats_unavailable);
        }
        final double rate = drainPercent * 60.0 * 60.0 * 1000.0 / durationMs;
